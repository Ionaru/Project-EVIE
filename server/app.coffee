...
if DEVMODE
  ...
  watch path.join(__dirname, '../client/style'), (filename) ->
    logger.debug("Changes detected in file #{filename}, recompiling.")
    await compiler.compileStylesheets(defer())
    return
  watch path.join(__dirname, '../client/scripts'), (filename) ->
    logger.debug("Changes detected in file #{filename}, recompiling.")
